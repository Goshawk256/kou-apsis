...
function AnaSayfa() {
  ...
  const getCurrentSemester = () => {
    ...
    if (currentMonth > 6) {
      ...
      return `${previousYear.toString().slice(-2)}${currentYear
        .toString()
        .slice(-2)}G`;
    }
  };
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const semesterData = lessonData?.data?.find(
          (lesson) => lesson.semester === currentSemester
        );
        ...
        const doctoralStudents =
          semesterData?.student_stats?.doctoral_students || 0;
        const mastersStudents =
          semesterData?.student_stats?.masters_students || 0;
  ...
  if (loading) {
    ...
    ); // Burada bir loading ekranı veya spinner gösterebilirsiniz
  }
  ...
  const publicationData = userInfo
    ? {
        ...
        datasets: [
          {
            label: "Yayın Sayısı",
            data: userInfo.userdata.lastFiveYears.map(
              (item) => item.publicationCount
            ),
    ...
    <div className={`main-anasayfa`}>
      ...
      <div className="anasayfa-row-1">
        ...
        <div className="anasayfa-row-1-card">
          ...
          <div className="card-inner-count">
            {userInfo.studentdata.masterStudentCount || "-"}
          </div>
        </div>
        <div className="anasayfa-row-1-card">
          <div className="card-row-1">
          ...
          <div className="card-inner-count">
            {userInfo.studentdata.phdStudentCount || "-"}
          </div>
        </div>
      </div>
      ...
      <div className="anasayfa-row-2">
        <div className="anasayfa-row-2-card">
          <div className="card-row-1">
            ...
            <label>
              Toplam Yayın Sayısı
              <strong>({userInfo.userdata.publicationCount})</strong>
            </label>
          </div>
        ...
        <div className="anasayfa-row-2-card">
          <div className="card-row-1">
            ...
            <label>
              Toplam Atıf Sayısı{" "}
              <strong>({userInfo.userdata.citationCount})</strong>
            </label>
          </div>
          {/* Atıf Sayısı Grafiği */}
